...
<Accordion title="How access control works on Olly? Is there special RBAC?">
    Olly users inherit their Coralogix access. Each user can access only data that is available to them in Coralogix.
</Accordion>

<Accordion title="Does Olly support regions and teams?">
    Yes - Olly supports both regions and teams. Each customer has an **Olly Organization**. Within an organization, they have all their Coralogix teams, and regions.
</Accordion>

<Accordion title="Can I access multiple teams within a single prompt?">
    Yes - when you send a message to Olly, it will use any team that’s accessible to answer and fulfil the task.
    ![Olly orgs](./images/olly-org.webp)
</Accordion>