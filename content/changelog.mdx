...
### 🤖 GPT Focus mode upgrade to GPT-5.1

Olly now runs Focus mode with GPT-5.1 (previously GPT-o3) when GPT is selected as the model provider.

This upgrade improves reasoning depth, consistency, and overall response quality during deeper investigations.
...
Olly now supports GitHub [integration](https://docs.olly.new/content/olly-github-integration), enabling code-aware investigations and root cause analysis directly from your repositories.

With GitHub connected, Olly can:

- Link logs, alerts, and incidents to the exact files and line ranges in your code.

- Analyze commits to identify regressions.

- Compare branches (for example, production vs. staging).

- Provide targeted, code-level fix suggestions.

This release helps teams move faster from detection to resolution by connecting production signals directly to source code.

![Github integration](./images/github-integration-update.png)
