...
Olly controls access to Coralogix data through data sources. Every question runs against one or more configured data sources that define which teams and telemetry Olly can query. Personal data sources keep the creator’s existing Coralogix permissions and policies and are visible only to that creator. An Olly admin creates organizational data sources and mirrors the permissions and policies associated with the key or token used to create them. This means that any data available to the creator of an organizational data source becomes available to every member that the admin shares that data source with.
...
4. Create either a personal or an organizational data source based on how you want to share access.

### Manual login

1. Sign in to Olly using your Gmail account. Olly uses the default region configured for your Olly organization.
2. Create either a personal or an organizational data source:
    - Use a personal API key from one of your Coralogix teams for a personal data source.
    - Use a team API key for an organizational data source.
...
Data sources are either personal or organizational:

- Personal data sources are accessible only to the member who created them.
- Organizational data sources are shared and can be created only by an Olly admin. Any member who is granted access to an organizational data source can retrieve data from it, within the limits of the permissions and policies applied to the key or token the admin used to create that data source.
...
When an organizational data source is used, anyone granted access to that data source can retrieve data from it. Admins should be aware that the data available to the creator of that data source will be available to all members who are granted access to it.

### Personal data sources

- Any Olly member can create a personal data source.
- A personal data source is private and can be used only by the member who created it.
- A personal data source can be created in two ways:
    - The user logs in to Olly using the login with Coralogix, approves access to their data, and Olly uses their access token to discover the Coralogix teams they belong to. Each of those teams becomes a personal data source the user can connect, and Olly can run on it.
    - The user manually enters a personal API key from one of their Coralogix teams.
- In both cases:
    - The personal data source exposes only the data the connecting user is allowed to access in Coralogix.
...
- Organizational data sources can be created only by Olly admins.
- An admin can decide whether an organizational data source is:
    - Available to all organization members, or
    - Available only to selected members.
- To create an organizational data source, an admin either:
    - Provides a Coralogix team API key, or
    - Creates the data source using an access token obtained via login with Coralogix.
- Members who are granted access to an organizational data source can use it as a data source for Olly to run on and generate answers.
- Members will have access only to the data permitted by the team API key or access token the admin used to create the organizational data source. Olly does not gain broader access than that key or token allows.
- Admins can change which members have access to an organizational data source at any time.
