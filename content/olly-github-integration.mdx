...
To connect the GitHub integration, both of the following requirements must be met:

- You are an **Olly organization admin**: Only organization admins can enable or disable the GitHub integration for the organization.
- You are an **admin in the GitHub organization** you are connecting.
...
### Installing without GitHub admin permissions

It is recommended that a GitHub organization admin perform the installation directly. However, you can still initiate the integration and request approval.

If you do not have GitHub organization admin permissions:

1. Go to **Connections → Integrations → GitHub**.
2. Toggle to **Connect**.
3. You will be redirected to GitHub.
4. Submit a request for approval for your GitHub organization admin.
5. A GitHub organization admin must approve the request.

After approval:

- The Olly GitHub integration is installed in the organization.
- Users in the Olly organization can create GitHub data sources as usual.

Until a GitHub admin approves the request, the integration remains **pending** and GitHub data sources cannot be created.
