...
<Accordion title="How do I define which users can access an organization in Olly">
    There are two ways to access Olly:

    1. **Auto-invite by domain** (recommended): If the organization has auto-invite by domain enabled, any user with an email from the approved domain can automatically join the organization. If a domain is missing, an admin can add it in the organization settings.
