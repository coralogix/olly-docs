...
To setup the Olly Slack integration, make sure the following requirements are met:
...
- **You use the correct region-specific Olly app and handle**: Add the Olly app to channels using `/invite @Olly-<region>` (for example, `/invite @Olly-EU2`) and always mention Olly using the matching region-specific handle.

## How to install

1. Go to **Settings → Connections → Integrations → Slack**.
2. Toggle to **Enable**.
3. Complete the Slack OAuth flow.
4. When prompted, choose which shared data source Slack should use. **Only shared data sources can be exposed to Slack**.
5. Save your changes.

![Enable Slack](./images/olly-integrations.png)

Once enabled, Olly answers using the selected data source.
...
To use Olly in Slack without interruptions, make sure the following requirements are met:
